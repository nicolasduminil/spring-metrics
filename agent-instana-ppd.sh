...
docker run \
  ...
  --env="INSTANA_AGENT_ZONE=preprod" \
  --env="INSTANA_TAGS=project=luqi,environment=preprod,location=FR-Paris,division=cision,contact=infrastructure.fr@cision.com,install=docker" \
